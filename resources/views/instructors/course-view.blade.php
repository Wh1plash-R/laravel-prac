<x-app-layout>
            ...
            <div class="mt-3">
                ...
                <form id="announcementForm" action="" method="POST" enctype="multipart/form-data" class="space-y-4"
                            ...
                            handleFileChange(event) {
                                ...
                                if (file) {
                                    ...
                                    if (!allowedTypes.includes(file.type)) {
                                        alert('Please select a valid file type (PDF, DOC, DOCX, TXT, ZIP, RAR, Images, Videos, PowerPoint, Excel).');
                                        event.target.value = '';
                                        return;
                                    }
                    ...
                    <div>
                        ...
                        <div x-show="currentFile" class="mb-3 p-3 bg-gray-50 border border-gray-200 rounded-md">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center flex-1">
                                    ...
                                    <div class="flex-1">
                                        <p class="text-sm text-gray-900 font-medium" x-text="currentFile?.name"></p>
                                        <p class="text-xs text-gray-500" x-text="currentFile ? formatFileSize(currentFile.size) + ' • Current file' : ''"></p>
                                    </div>
                                </div>
                        ...
                        <!-- File Upload Area -->
                        <div id="announcement-file-upload-area" class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md hover:border-gray-400 transition-colors">
                        ...
                        <div x-show="selectedFile" class="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-md">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center flex-1">
                                    ...
                                    <div class="flex-1">
                                        <p class="text-sm text-gray-900 font-medium" x-text="selectedFile?.name"></p>
                                        <p class="text-xs text-gray-500" x-text="selectedFile ? formatFileSize(selectedFile.size) + ' • Selected for upload' : ''"></p>
                                    </div>
                                </div>
                                <button type="button" @click="selectedFile = null; hasChanges = true" class="text-red-600 hover:text-red-800 text-sm font-medium ml-3">
                                    Remove
                                </button>
                            </div>
                        </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Initialize announcement modal drag and drop functionality
            initializeAnnouncementDragAndDrop();
        });

        // Announcement modal drag and drop functionality
        function initializeAnnouncementDragAndDrop() {
            const fileUploadArea = document.getElementById('announcement-file-upload-area');
            const fileInput = document.getElementById('announcement_file');
            ...
            function highlight(e) {
                fileUploadArea.classList.add('border-blue-500', 'border-solid', 'bg-blue-50');
            }

            function unhighlight(e) {
                fileUploadArea.classList.remove('border-blue-500', 'border-solid', 'bg-blue-50');
            }
